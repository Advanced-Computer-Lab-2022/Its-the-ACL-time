...
import { updateProgress } from '../store/slices/myCourses_slice';
...
const SubtitlesPage = () => {
  ...
  useEffect(() => {
    setLoading(
      coursesIsLoading || myCoursesIsLoading || subtitlesLoading || examsLoading
    );
  }, [coursesIsLoading, myCoursesIsLoading, subtitlesLoading, examsLoading]);
  ...
  useEffect(() => {
    if (!loading && !myCoursesIsLoading) {
      checkOwnership();
    }
  }, [myCoursesIsLoading, loading]);
  ...
  useEffect(() => {
    ...
    async function getProgress() {
      const { courseProgress } = myCourses?.find(
        (course) => course._id === courseId
      );
      const { progress, completedSubtitles, completedExams, reviewed } =
        courseProgress || {};
      setState({
        checkedSubtitles: completedSubtitles || [],
        checkedExams: completedExams || [],
        progress: progress || 0,
      });
    ...
    if (!myCoursesIsLoading && myCourses.length > 0) {
      getProgress();
    }
  }, [courseId, token, myCoursesIsLoading, myCourses]);
  ...
  }, [searchParams, user, token, videoInfo, courseId]);
  ...
  const updateCourseProgress = async (checkedSubtitles, checkedExams) => {
    ...
    dispatch(updateProgress({ ...tmp }));
  };

  const handleChecked = async (subtitle, title, checked) => {
    setState((prevState) => {
      if (checked) {
        prevState.checkedSubtitles = [...prevState.checkedSubtitles, subtitle];
      } else {
    ...
    updateCourseProgress(state.checkedSubtitles, state.checkedExams);
  };

  const handleCheckedExam = async (exam, title, checked) => {
    setState((prevState) => {
      if (checked) {
        prevState.checkedExams = [...prevState.checkedExams, exam];
      } else {
